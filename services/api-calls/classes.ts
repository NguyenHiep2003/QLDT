    ...
    try {
        ...
        if (error.rawError) {
            ...
            if (errorCode == '1004') {
                ...
                error.setContent(
                    'Thời gian tra cứu không thuộc thời gian mở lớp'
                );
            }
...
export async function createClass(request: createClassRequest) {
    ...
    try {
        const response: createClassResponse = await instance.post(
...
export async function getClassInfo(request: getClassInfoRequest) {
    ...
    try {
        const response: getClassInfoResponse = await instance.post(
...
export async function editClass(request: editClassRequest) {
    ...
    try {
        const response: editClassResponse = await instance.post(
            '/it5023e/edit_class',
            {
                class_id: request.class_id,
                class_name: request.class_name,
...
export async function requestAbsence(form: FormData) {
    ...
    try {
        ...
        const response = await instance.post('/it5023e/request_absence', form, {
            headers: {
                'no-need-token': true,
...
export async function getStudentAbsenceRequests(
    class_id: any = null,
    page: any = null,
    page_size: any = null,
    status: any = null,
    ...
    try {
        ...
        if (error.rawError) {
            ...
            if (errorCode == '1009') {
                error.setTitle('Lỗi');
                error.setContent(
                    'Giảng viên không có quyền xem xét đơn xin nghỉ này!'
                );
            } else if (errorCode == '9999') {
                //warning: error code should be 9994
                error.setTitle('Lỗi');
...
export async function getOpenClassList(
    pageable_request: any
): Promise<getClassOpenResponse | { meta: { code: number; message: string } }> {
    const profile = await getProfileLocal();
    if (!profile) return { meta: { code: 400, message: 'Profile not found' } };

    try {
        const response: getClassOpenResponse = await instance.post(
            '/it5023e/get_open_classes',
            {
                pageable_request: pageable_request,
            }
        );
...
export async function getOpenClassByFilter(
    request: any
): Promise<getClassOpenResponse | { meta: { code: number; message: string } }> {
    const profile = await getProfileLocal();
    if (!profile) return { meta: { code: 400, message: 'Profile not found' } };
...
export async function getAttendanceDates(class_id: any) {
    //SGHB - checked
    const profile = await getProfileLocal();
    if (!profile) return { meta: { code: 400, message: 'Profile not found' } };
