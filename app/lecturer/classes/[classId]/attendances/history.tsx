...
const handleLookup = async () => {
    ...
    try{
        ...
        if (error.response) {
            ...
            else if(errorCode == 1004) setErr(`Thời gian tra cứu\nkhông thuộc thời gian mở lớp!`)
            setIsLoading(false)
            setShowErr(true)
...
return (
    <SafeAreaView style={styles.container}>
        ...
        {showRecord && <FlatList
            ...
            data={data}
            renderItem={({item, index}) => <Item name={`${item.first_name} ${item.last_name}`} MSSV={item.student_id} status={item.status} index={index}/>}
