...
const AssignmentScreen = () => {
  ...
  useEffect(() => {
    const loadSubmission = async () => {
      try {
        ...
        console.log(result)
        setStudentResponse(result?.text_response || "");
      } catch (err: any) {
  ...
  const confirmSubmission = () => {
    Alert.alert(
      "Xác nhận nộp bài",
      "Bạn chỉ được phép nộp bài một lần duy nhất. Bạn có chắc chắn muốn nộp?",
      [
  ...
  const handleSubmitAssignment = async () => {
    ...
    try {
      ...
      setSubmission({
        submission_time: new Date().toISOString().split('.')[0],
        text_response: studentResponse,
        file_url: selectedFile?.uri,
        grade: null, // Điểm mặc định chưa chấm
      });
      console.log(submission.submission_time)
          ...
          {loading ? (
            ...
            <>
              ...
              <Text style={[styles.submissionText, { color: "#4caf50" }]}>
                Nộp lúc: {new Date(submission.submission_time + 'Z').toLocaleString("vi-VN", {
      timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone})}
              </Text>
              {/* Điểm */}
